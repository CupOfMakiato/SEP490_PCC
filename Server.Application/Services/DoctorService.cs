...
namespace Server.Application.Services
{
    public class DoctorService : IDoctorService
    {
        ...
        public async Task<Result<List<ViewDoctorDTO>>> GetDoctorsByClinicIdAsync(Guid clinicId)
        {
            var doctors = _mapper.Map<List<ViewDoctorDTO>>(
                await _doctorRepository.GetDoctorsByClinicIdAsync(clinicId));

            return new Result<List<ViewDoctorDTO>>
            {
                Error = doctors.Count == 0 ? 1 : 0,
                Message = doctors.Count == 0 ? "No doctors found for this clinic" : "View doctors by clinic successfully",
                Data = doctors
            };
        }
